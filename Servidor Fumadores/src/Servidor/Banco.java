...
public class Banco {
	...
	//Método que devuelve la hora actual del servidor.
	...
	//Método que agrega ingredientes al banco.
	public synchronized void nuevosIngredientes (int ingredienteNuevo) throws  InterruptedException {
		//Bucle infinito que detiene al vendedor de poner mas ingredientes en el banco si algun fumador está fumando o el banco ya tiene ingredientes.
		while(hayIngredientes || fumando){
			wait();
		}
		//Se asigna el número de ingrediente aleatorio seleccionado por el vendedor al banco.
		ingredientes = ingredienteNuevo;
		//Se especifica que el banco se encuentra lleno.
		hayIngredientes = true;
		//El número aleatorio indica que ingrediente NO va a ser añadido al banco.
	...
	public synchronized void fumar (int idUsuario)  throws  InterruptedException {
		...
		switch(idUsuario){
			case 0:
				System.out.println("Fumador(Tabaco) - Ha recogido papel y fósforo del banco.");
				//Se registra el evento en la traza de tipo XML.
		...
		//Se indica que el banco quedó vacío.
	...
	public synchronized void dejarFumar (int idUsuario) {
		...
		//Se notifica a los demás fumadores y al vendedor que el fumador ha dejado de fumar.
		notifyAll();
	}
